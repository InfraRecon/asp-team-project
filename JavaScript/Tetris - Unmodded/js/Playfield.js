class Playfield {
	...
	show() {
		...
		if (this.gridlines) this.p.fill(this.background);
		else fill(this.foreground);
		
		this.p.stroke(this.background)
		this.p.strokeWeight(bs);

		// offset the rectangle so that
		// top and right borders stay in canvas
		let offset = this.p.floor(bs / 2)
		this.p.rect(offset, offset, cs * this.cols + bs - 1, cs * this.rows + bs - 1)
		...
		for (let row = 0; row < this.grid.length; row++) {
			for (let col = 0; col < this.grid[row].length; col++) {
				...
				this.p.fill(this.grid[row][col]);
				
				this.p.noStroke();
				this.p.rect(cs * col + offset, cs * row + offset, cs - 1, cs - 1);
			}
		}
