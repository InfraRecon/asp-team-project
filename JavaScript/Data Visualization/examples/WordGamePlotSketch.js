...
{   
    ...
    p.graphSetup = function()
    {
		...
		for (var i = 0; i < timeData.length; i++) 
        {
            ...
            wordsRightPoints[i] = new GPoint(i, p.round(rightData[i]));
            wordsWrongPoints[i] = new GPoint(i, p.round(wrongData[i]));
		}
		...
		plot.setTitleText("Word Game Acheivements");
        
        plot.addLayer("layer 1", wordsRightPoints);
		plot.getLayer("layer 1").setLineColor(p.color(255, 150, 0));
        
        plot.addLayer("layer 2", wordsWrongPoints);
		...
		plot1.setTitleText("Word Game Multiple Plots");
		plot1.getTitle().setRelativePos(1);
		plot1.getTitle().setTextAlignment(p.CENTER);

		plot2.setPoints(wordsRightPoints);

		plot3.setPoints(wordsWrongPoints);
		...
		periodPlot.addLayer("Words Right", wordsRightPoints);
		periodPlot.getLayer("Words Right").setLineColor(p.color(255, 100, 255));
